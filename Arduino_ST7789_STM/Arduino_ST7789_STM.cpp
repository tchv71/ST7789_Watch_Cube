...
void Arduino_ST7789::init (uint16_t width, uint16_t height)
{
  ...
  SPI.beginTransaction (
      SPISettings (SPI_FREQ, MSBFIRST, SPI_MODE3, DATA_SIZE_8BIT));
                                    ...
                                    uint16_t y1)
{
  ...
  DC_COMMAND;
  SPI.write (ST7789_CASET);
  DC_DATA;
  SPI.write (x0);
  SPI.write (x1);

  DC_COMMAND;
  SPI.write (ST7789_RASET);
  DC_DATA;
  SPI.write (y0);
  SPI.write (y1);

  DC_COMMAND;
  SPI.write (ST7789_RAMWR);
...
void Arduino_ST7789::drawImage (int16_t x, int16_t y, int16_t w, int16_t h,
                                uint16_t *img16)
{
  if (x >= _width || y >= _height || w <= 0 || h <= 0)
    return;
...
void Arduino_ST7789::drawImageF (int16_t x, int16_t y, int16_t w, int16_t h,
